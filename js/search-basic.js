...
function search() {
	...
	$("#search-results").html("<span id=\"loading\"><img src=\"[system-asset]/files/images/loading.gif[/system-asset]\"/>Searching...</span>");
	...
	$.get('[system-view:internal]/render/page.act?id=449b5d3f0a00024e12439978feafa2b3&type=page[/system-view:internal][system-view:external]blogs.xml[/system-view:external]', function(xml) {		
		...
		for(var x=0; x < blogs.page.length; x++) {
			...
			if (blogs.page[x].title != '' && (blogs.page[x].keywords.toLowerCase().indexOf(search_term.toLowerCase()) != -1 || blogs.page[x].title.toLowerCase().indexOf(search_term.toLowerCase()) != -1)) {			
				results.append("<li><a href=\""+blogs.page[x].path+"\">" + blogs.page[x].title+ "</a></li>");
				count++;
			}							
		}
